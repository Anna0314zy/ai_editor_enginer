...
- [GlobalSettingsPanel.tsx](file://src/components/GlobalSettingsPanel.tsx)
</cite>
...
- 状态同步机制与与编辑器视图层的协作
- **新增**：页面背景与安全区域管理功能，支持全局设置面板与页面级背景覆盖
- 与其他组件（动画引擎、吸附系统）的关系
- 常见问题与最佳实践
...
- **新增**：components/GlobalSettingsPanel.tsx 全局设置面板，支持页面背景与安全区域管理
- **新增**：components/PropertyPanel.tsx 页面属性面板，支持页面级背景覆盖
- **新增**：components/Canvas.tsx 画布组件，集成安全区域可视化与背景渲染
...
DocBgCmd["UpdateDocumentBackgroundCommand<br/>更新文档背景"]
DocSafeCmd["UpdateDocumentSafeAreaCommand<br/>更新文档安全区域"]
end
subgraph "类型与动画"
...
Canvas["Canvas.tsx<br/>画布组件<br/>安全区域可视化"]
GlobalSettings["GlobalSettingsPanel.tsx<br/>全局设置面板"]
PropertyPanel["PropertyPanel.tsx<br/>页面属性面板"]
end
Engine --> Scene
Engine --> History
Engine --> Timeline
...
**图表来源**
- [engine.ts:1-138](file://src/engine/engine.ts#L1-L138)
- [scene.ts:1-288](file://src/engine/scene.ts#L1-L288)
- [history.ts:1-44](file://src/engine/history.ts#L1-L44)
- [timeline.ts:1-66](file://src/engine/timeline.ts#L1-L66)
- [commands.ts:1-370](file://src/engine/commands.ts#L1-L370)
...
- [index.ts:1-191](file://src/types/index.ts#L1-L191)
...
- [Canvas.tsx:1-248](file://src/components/Canvas.tsx#L1-L248)
- [GlobalSettingsPanel.tsx:1-136](file://src/components/GlobalSettingsPanel.tsx#L1-L136)
...
- Engine
  - 聚合场景、历史与时间轴，提供统一入口
  - **新增**：updateDocumentBackground(background: PageBackground)：更新文档全局背景
  - **新增**：updateDocumentSafeArea(safeArea: SafeArea)：更新文档安全区域设置
...
- Scene
  ...
  - **新增**：updateDocumentBackground(background: PageBackground)：更新文档全局背景
  - **新增**：updateDocumentSafeArea(safeArea: SafeArea)：更新文档安全区域
...
**章节来源**
- [engine.ts:88-96](file://src/engine/engine.ts#L88-L96)
...
- [scene.ts:14-20](file://src/engine/scene.ts#L14-L20)
- [scene.ts:263-282](file://src/engine/scene.ts#L263-L282)
...
- [engine.ts:98-103](file://src/engine/engine.ts#L98-L103)
...
- 关键方法
  ...
  - **新增**：updateDocumentBackground(background): 更新文档全局背景
  - **新增**：updateDocumentSafeArea(safeArea): 更新文档安全区域
- 设计要点
...
**图表来源**
- [engine.ts:15-138](file://src/engine/engine.ts#L15-L138)
...
- 文档结构
  ...
  - **新增**：background 全局背景设置，默认纯白色
  - **新增**：safeArea 安全区域设置，默认无安全区域
- 页面管理
...
- **新增**：文档级背景管理
  - updateDocumentBackground(background: PageBackground)：更新全局背景
  - 默认背景为纯白色实色背景
- **新增**：安全区域管理
  - updateDocumentSafeArea(safeArea: SafeArea)：更新全局安全区域
  - 默认安全区域为 { top: 0, right: 0, bottom: 0, left: 0 }
- 复杂度
...
- [scene.ts:14-20](file://src/engine/scene.ts#L14-L20)
- [scene.ts:263-282](file://src/engine/scene.ts#L263-L282)
- [scene.ts:3-288](file://src/engine/scene.ts#L3-L288)
...
- 典型命令
  ...
  - **新增**：文档背景命令：UpdateDocumentBackgroundCommand
  - **新增**：文档安全区域命令：UpdateDocumentSafeAreaCommand
- 批量动画命令
...
UpdateDocumentBackgroundCommand ..|> Command
UpdateDocumentSafeAreaCommand ..|> Command
```

**图表来源**
- [commands.ts:4-370](file://src/engine/commands.ts#L4-L370)
...
### 页面背景与安全区域管理
- **页面背景管理**
  - 支持三种背景类型：实色背景、渐变背景、图片背景
  - 全局背景设置：通过 GlobalSettingsPanel.tsx 配置
  - 页面级覆盖：通过 PropertyPanel.tsx 设置页面自定义背景
  - 背景样式渲染：Canvas.tsx 中的 getBackgroundStyle 函数处理不同类型的背景渲染
- **安全区域管理**
  - 安全区域定义：上下左右四个方向的安全边距
  - 预设选项：标准、宽幅、动作安全区域等常用预设
  - 可视化显示：Canvas.tsx 中绘制不安全区域的半透明红色遮罩和安全边界虚线
  - 全局设置：通过 GlobalSettingsPanel.tsx 配置安全区域
- **类型定义**
  - PageBackground：包含 solid、gradient、image 三种类型
  - SafeArea：包含 top、right、bottom、left 四个数值属性
  - Document：新增 background 和 safeArea 字段
- **命令支持**
  - UpdateDocumentBackgroundCommand：更新文档背景的撤销/重做支持
  - UpdateDocumentSafeAreaCommand：更新文档安全区域的撤销/重做支持

```mermaid
flowchart TD
Doc["Document<br/>background: PageBackground<br/>safeArea: SafeArea"] --> GlobalPanel["GlobalSettingsPanel<br/>全局背景/安全区域设置"]
Doc --> PagePanel["PropertyPanel<br/>页面级背景覆盖"]
Doc --> Canvas["Canvas<br/>背景渲染/安全区域可视化"]
GlobalPanel --> Engine["Engine<br/>updateDocumentBackground()<br/>updateDocumentSafeArea()"]
Engine --> Scene["Scene<br/>updateDocumentBackground()<br/>updateDocumentSafeArea()"]
Scene --> History["History<br/>撤销/重做支持"]
Canvas --> Types["types/index.ts<br/>PageBackground/SafeArea 类型"]
```

**图表来源**
- [GlobalSettingsPanel.tsx:32-136](file://src/components/GlobalSettingsPanel.tsx#L32-L136)
- [PropertyPanel.tsx:37-91](file://src/components/PropertyPanel.tsx#L37-L91)
- [Canvas.tsx:12-32](file://src/components/Canvas.tsx#L12-L32)
- [Canvas.tsx:148-173](file://src/components/Canvas.tsx#L148-L173)
- [engine.ts:88-96](file://src/engine/engine.ts#L88-L96)
- [scene.ts:14-20](file://src/engine/scene.ts#L14-L20)
...
- **与界面组件的集成**
  - GlobalSettingsPanel.tsx：全局背景和安全区域设置面板
  - PropertyPanel.tsx：页面属性面板，支持页面级背景覆盖
  - Canvas.tsx：画布组件，集成背景渲染和安全区域可视化
...
- 内部依赖
  ...
  - **新增**：界面组件依赖 Engine 提供的背景和安全区域管理接口
- 外部依赖
...
Engine --> GlobalSettings["GlobalSettingsPanel"]
Engine --> PropertyPanel["PropertyPanel"]
...
- [GlobalSettingsPanel.tsx:1-5](file://src/components/GlobalSettingsPanel.tsx#L1-L5)
- [PropertyPanel.tsx:1-5](file://src/components/PropertyPanel.tsx#L1-L5)
- [Canvas.tsx:1-10](file://src/components/Canvas.tsx#L1-L10)
...
- **新增**：页面背景渲染优化
  - 背景样式计算通过 getBackgroundStyle 函数集中处理，避免重复计算
  - 安全区域可视化采用 CSS 遮罩层，渲染性能良好
...
- **新增**：页面背景与安全区域问题
  - **背景不显示**：检查 getBackgroundStyle 函数中的类型判断和默认值处理
  - **安全区域不生效**：确认 Canvas 组件中安全区域的 CSS 样式计算
  - **全局设置面板无响应**：检查 GlobalSettingsPanel.tsx 中的回调函数绑定
  - **页面级背景覆盖无效**：确认 PropertyPanel.tsx 中的页面背景更新逻辑
...
- [animationStore.ts:21-26](file://src/store/animationStore.ts#L21-L26)
- [Canvas.tsx:12-32](file://src/components/Canvas.tsx#L12-L32)
- [GlobalSettingsPanel.tsx:38-50](file://src/components/GlobalSettingsPanel.tsx#L38-L50)
- [PropertyPanel.tsx:72-78](file://src/components/PropertyPanel.tsx#L72-L78)

## 结论
该引擎系统以命令模式为核心，实现了可追踪、可撤销的状态变更；Scene 提供了完整的文档与元素生命周期管理；History 保障了回溯能力；Timeline 提供了基础的时间轴播放控制；**最新版本引入的 Store 精准订阅模式通过 EngineTopic 分类通知机制，显著提升了状态管理的性能和可扩展性**；**新增的页面背景与安全区域管理功能，通过 GlobalSettingsPanel 和 PropertyPanel 提供了完整的全局设置和页面级覆盖能力，配合 Canvas 组件的可视化渲染，形成了从设计到呈现的一体化工作流**；与动画引擎、吸附系统协同，形成从数据到表现的一体化架构。通过框架无关的设计和 Store 系统，系统具备良好的扩展性与可维护性。
...
- Scene
  ...
  - **新增**：文档背景：updateDocumentBackground(background)
  - **新增**：安全区域：updateDocumentSafeArea(safeArea)
- History
...
- **新增**：类型定义
  - PageBackground：实色背景、渐变背景、图片背景
  - SafeArea：上下左右安全区域
  - Document：新增 background 和 safeArea 字段

**章节来源**
- [engine.ts:33-138](file://src/engine/engine.ts#L33-L138)
- [scene.ts:10-288](file://src/engine/scene.ts#L10-L288)
...
- [editorUIStore.ts:11-53](file://src/store/editorUIStore.ts#L11-L53)
- [pluginStore.ts:24-100](file://src/store/pluginStore.ts#L24-L100)
- [index.ts:72-116](file://src/types/index.ts#L72-L116)
...
- **新增**：页面背景与安全区域使用模式
  - **全局背景设置**：通过 GlobalSettingsPanel.tsx 设置文档级背景，所有页面继承
  - **页面级覆盖**：通过 PropertyPanel.tsx 为特定页面设置自定义背景，覆盖全局设置
  - **安全区域配置**：通过 GlobalSettingsPanel.tsx 设置安全区域，Canvas 组件实时可视化
  - **背景类型选择**：支持实色、渐变、图片三种背景类型，注意性能优化
  - **安全区域预设**：使用标准、宽幅、动作安全区域等预设，提高工作效率
