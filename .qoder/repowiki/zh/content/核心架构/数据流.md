# 数据流

<cite>
**本文引用的文件**
- [src/engine/index.ts](file://src/engine/index.ts)
- [src/engine/engine.ts](file://src/engine/engine.ts)
- [src/engine/scene.ts](file://src/engine/scene.ts)
- [src/engine/history.ts](file://src/engine/history.ts)
- [src/engine/commands.ts](file://src/engine/commands.ts)
- [src/engine/timeline.ts](file://src/engine/timeline.ts)
- [src/types/index.ts](file://src/types/index.ts)
- [src/types/animation.ts](file://src/types/animation.ts)
- [src/App.tsx](file://src/App.tsx)
- [src/main.tsx](file://src/main.tsx)
- [src/components/Canvas.tsx](file://src/components/Canvas.tsx)
- [src/animation/engine.ts](file://src/animation/engine.ts)
- [src/animation/scheduler.ts](file://src/animation/scheduler.ts)
- [src/animation/index.ts](file://src/animation/index.ts)
- [src/store/baseStore.ts](file://src/store/baseStore.ts)
- [src/store/readme.md](file://src/store/readme.md)
- [src/store/hooks.ts](file://src/store/hooks.ts)
- [src/store/sceneStore.ts](file://src/store/sceneStore.ts)
- [src/store/selectionStore.ts](file://src/store/selectionStore.ts)
- [src/store/editorUIStore.ts](file://src/store/editorUIStore.ts)
- [src/store/historyStore.ts](file://src/store/historyStore.ts)
- [src/store/animationStore.ts](file://src/store/animationStore.ts)
- [src/store/pluginStore.ts](file://src/store/pluginStore.ts)
- [src/hooks/useEngineSnapshot.ts](file://src/hooks/useEngineSnapshot.ts)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [精确订阅理论](#精确订阅理论)
6. [详细组件分析](#详细组件分析)
7. [依赖关系分析](#依赖关系分析)
8. [性能考量](#性能考量)
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)
11. [附录](#附录)

## 简介
本技术文档围绕 AI 课件编辑器的数据流系统，系统性阐述从用户交互到 UI 更新的完整数据流路径：命令执行、状态变更与视图渲染；解释 Scene 场景管理如何维护文档状态，Engine 引擎如何协调各子系统，以及 AnimationEngine 如何同步动画状态；并提供元素编辑、动画配置与历史记录的具体处理示例，说明数据一致性保证机制、状态同步策略与性能优化方案，同时覆盖数据验证、错误处理与回滚机制。

**更新**：本版本重点介绍了精确订阅理论和实现，包括Engine-level、Store-level和React-level三个层级的订阅隔离机制，替代原有的全局订阅模式。

## 项目结构
编辑器采用"引擎 + 视图"的分层设计：
- 引擎层（engine）：无框架绑定，负责文档状态（Scene）、命令执行（History）、时间轴（Timeline）与命令集（Commands）。
- 存储层（store）：基于精确订阅理论的Store体系，提供主题化的状态管理。
- 动画层（animation）：独立于引擎，通过适配器（WebAnimationAdapter/GSAPAdapter）驱动播放生命周期。
- 类型层（types）：统一定义元素、页面、动画配置等类型。
- 视图层（React 组件）：Canvas、属性面板、动画面板等，负责事件捕获、状态刷新与调度动画。

```mermaid
graph TB
subgraph "视图层"
APP["App.tsx"]
CANVAS["Canvas.tsx"]
end
subgraph "存储层"
BASESTORE["BaseStore & EngineStore"]
SCENESTORE["SceneStore"]
SELECTIONSTORE["SelectionStore"]
EDITORUISTORE["EditorUIStore"]
HISTORYSTORE["HistoryStore"]
ANIMATIONSTORE["AnimationStore"]
PLUGNSTORE["PluginStore"]
HOOKS["hooks.ts"]
END
subgraph "引擎层"
ENGINE["Engine(engine.ts)"]
SCENE["Scene(scene.ts)"]
HISTORY["History(history.ts)"]
TIMELINE["Timeline(timeline.ts)"]
CMDS["Commands(commands.ts)"]
end
subgraph "动画层"
ANIM_ENGINE["AnimationEngine(engine.ts)"]
SCHED["AnimationScheduler(scheduler.ts)"]
ADAPTER["WebAnimationAdapter / GSAPAdapter"]
end
TYPES["types/index.ts / types/animation.ts"]
APP --> CANVAS
APP --> BASESTORE
APP --> HOOKS
CANVAS --> ENGINE
BASESTORE --> ENGINE
ENGINE --> SCENE
ENGINE --> HISTORY
ENGINE --> TIMELINE
ENGINE --> CMDS
ANIM_ENGINE --> ADAPTER
APP -.-> SCHED
SCHED --> ANIM_ENGINE
ENGINE -.-> TYPES
ANIM_ENGINE -.-> TYPES
CANVAS -.-> TYPES
```

**图表来源**
- [src/App.tsx:19-335](file://src/App.tsx#L19-L335)
- [src/store/baseStore.ts:1-51](file://src/store/baseStore.ts#L1-L51)
- [src/store/hooks.ts:1-32](file://src/store/hooks.ts#L1-L32)
- [src/engine/engine.ts:15-128](file://src/engine/engine.ts#L15-L128)

**章节来源**
- [src/App.tsx:19-335](file://src/App.tsx#L19-L335)
- [src/components/Canvas.tsx:22-191](file://src/components/Canvas.tsx#L22-L191)
- [src/engine/index.ts:1-16](file://src/engine/index.ts#L1-L16)

## 核心组件
- Engine：集中式执行入口，封装 Scene、History、Timeline，提供 execute/undo/redo 能力，支持精确订阅。
- Scene：文档状态容器，提供页面、节点、元素与动画的增删改查与层级关系维护。
- History：命令栈，支持撤销/重做与清空。
- Commands：面向具体业务的命令对象，封装可逆操作。
- AnimationEngine：动画生命周期控制器，注册/播放/停止/暂停/重置，委托适配器执行。
- AnimationScheduler：基于"点击步骤 + 并发批次"的编排器，实现分步播放与并发控制。
- Store体系：基于精确订阅理论的Store容器，提供主题化的状态管理。
- 类型系统：统一元素、页面、动画配置与调度模型的类型定义。

**章节来源**
- [src/engine/engine.ts:15-128](file://src/engine/engine.ts#L15-L128)
- [src/engine/scene.ts:3-273](file://src/engine/scene.ts#L3-L273)
- [src/engine/history.ts:3-45](file://src/engine/history.ts#L3-L45)
- [src/engine/commands.ts:4-280](file://src/engine/commands.ts#L4-L280)
- [src/animation/engine.ts:9-120](file://src/animation/engine.ts#L9-L120)
- [src/animation/scheduler.ts:56-160](file://src/animation/scheduler.ts#L56-L160)
- [src/store/baseStore.ts:6-50](file://src/store/baseStore.ts#L6-L50)
- [src/types/index.ts:107-159](file://src/types/index.ts#L107-L159)
- [src/types/animation.ts:26-113](file://src/types/animation.ts#L26-L113)

## 架构总览
编辑器遵循"命令驱动 + 精确订阅 + 状态隔离"的架构原则：
- 所有状态变更必须通过 Engine.execute(command) 进入历史栈，确保可回溯。
- EditorState（视口、工具模式、选中集合等）与 Scene 文档状态分离，避免耦合。
- 动画系统独立于引擎，通过 AnimationEngine 与适配器解耦，支持多播放后端。
- **新增**：采用三层订阅隔离机制，确保组件只在关心的数据变化时重渲染。

```mermaid
classDiagram
class Engine {
+scene : Scene
+history : History
+timeline : Timeline
+subscribe(topic, callback)
+unsubscribe(topic, callback)
+notify(topic)
+execute(command)
+undo()
+redo()
+getEditorState()
+setEditorState(updates)
}
class Scene {
+getDocument()
+addPage()/removePage()
+addNode()/removeNode()
+addElement()/updateElement()/deleteElement()
+addAnimation()/removeAnimation()/updateAnimation()/reorderAnimations()
+getPageElements()/getPageAnimations()
}
class History {
+push(command)
+undo()
+redo()
+clear()
}
class Timeline {
+getCurrentTime()
+getDuration()
+isPlaying()
+play()/pause()/seek()
}
class AnimationEngine {
+register()/unregister()
+getAllConfigs()/getConfig()
+play()/playAllForElement()
+stop()/stopAll()/pause()/resume()
+reset()
}
class BaseStore {
+subscribe(callback)
+getSnapshot()
+emit()
}
class EngineStore {
+constructor(engine, topics)
+buildSnapshot()
}
class SceneStore {
+buildSnapshot()
+getElement()
+getPage()
+getPageElements()
+getPageAnimations()
}
class SelectionStore {
+buildSnapshot()
+select()
+multiSelect()
+addToSelection()
+removeFromSelection()
+toggleSelection()
+clear()
}
class EditorUIStore {
+buildSnapshot()
+setViewport()
+setToolMode()
+setHoveredElementId()
+zoomIn()
+zoomOut()
+resetZoom()
}
class HistoryStore {
+buildSnapshot()
+undo()
+redo()
+clear()
}
class AnimationStore {
+buildSnapshot()
+getAnimation()
+getAnimationsByElementId()
+play()
+pause()
+seek()
}
Engine --> Scene : "持有"
Engine --> History : "持有"
Engine --> Timeline : "持有"
BaseStore <|-- EngineStore : "继承"
EngineStore <|-- SceneStore : "继承"
EngineStore <|-- SelectionStore : "继承"
EngineStore <|-- EditorUIStore : "继承"
EngineStore <|-- HistoryStore : "继承"
EngineStore <|-- AnimationStore : "继承"
AnimationEngine --> Scene : "读取动画配置"
```

**图表来源**
- [src/engine/engine.ts:15-128](file://src/engine/engine.ts#L15-L128)
- [src/engine/scene.ts:3-273](file://src/engine/scene.ts#L3-L273)
- [src/engine/history.ts:3-45](file://src/engine/history.ts#L3-L45)
- [src/engine/timeline.ts:1-66](file://src/engine/timeline.ts#L1-L66)
- [src/animation/engine.ts:9-120](file://src/animation/engine.ts#L9-L120)
- [src/store/baseStore.ts:6-50](file://src/store/baseStore.ts#L6-L50)
- [src/store/sceneStore.ts:15-59](file://src/store/sceneStore.ts#L15-L59)
- [src/store/selectionStore.ts:12-69](file://src/store/selectionStore.ts#L12-L69)
- [src/store/editorUIStore.ts:11-53](file://src/store/editorUIStore.ts#L11-L53)
- [src/store/historyStore.ts:9-35](file://src/store/historyStore.ts#L9-L35)
- [src/store/animationStore.ts:13-60](file://src/store/animationStore.ts#L13-L60)

## 精确订阅理论
编辑器实现了三层订阅隔离机制，完全替代了原有的全局订阅模式：

### Engine-level 精确订阅
Engine支持按主题（topic）进行订阅，提供精确的消息分发：
- 主题类型：'scene' | 'editorState' | 'history' | 'all'
- notify方法现在按主题分发，只触发相关主题的订阅者
- 支持全局订阅和主题订阅两种模式

### Store-level 精确订阅
每个Store实例化时指定要监听的主题列表，确保主题级别的物理隔离：
- SceneStore监听['scene']主题
- SelectionStore监听['editorState']主题  
- EditorUIStore监听['editorState']主题
- HistoryStore监听['history']主题
- AnimationStore监听['scene']主题并订阅timeline

### React-level 精确订阅
使用React的useSyncExternalStore Hook，只在Store的回调触发时重渲染：
- 组件只订阅自己关心的Store
- 避免了全局订阅导致的无谓重渲染
- 提供了精确的状态同步

```mermaid
sequenceDiagram
participant U as "用户"
participant C as "Canvas.tsx"
participant E as "Engine"
participant SS as "SelectionStore"
participant HS as "HistoryStore"
participant SC as "SceneStore"
U->>C : "拖拽元素"
C->>E : "execute(MoveElementCommand)"
E->>E : "notify('scene')"
E->>SS : "不会触发主题不匹配"
E->>HS : "不会触发主题不匹配"
E->>SC : "触发 SceneStore.emit()"
SC->>SC : "buildSnapshot()"
SC-->>C : "UI刷新"
```

**图表来源**
- [src/engine/engine.ts:62-72](file://src/engine/engine.ts#L62-L72)
- [src/store/sceneStore.ts:17](file://src/store/sceneStore.ts#L17)
- [src/store/selectionStore.ts:14](file://src/store/selectionStore.ts#L14)
- [src/store/historyStore.ts:11](file://src/store/historyStore.ts#L11)

**章节来源**
- [src/engine/engine.ts:7-72](file://src/engine/engine.ts#L7-L72)
- [src/store/baseStore.ts:39-49](file://src/store/baseStore.ts#L39-L49)
- [src/store/readme.md:5-46](file://src/store/readme.md#L5-L46)
- [src/store/hooks.ts:9-31](file://src/store/hooks.ts#L9-L31)

## 详细组件分析

### 命令执行与历史回滚（元素编辑）
- 用户在画布拖拽或键盘删除触发命令，Engine.execute(command) 将命令入栈并执行。
- 撤销/重做时，History 弹出命令并调用其 undo()/execute()，实现精确回退。
- 元素编辑命令（如 MoveElementCommand）在执行前保存"之前值"，undo 时恢复。

```mermaid
sequenceDiagram
participant U as "用户"
participant C as "Canvas.tsx"
participant E as "Engine"
participant H as "History"
participant S as "Scene"
U->>C : "拖拽/键盘删除"
C->>E : "execute(AddElementCommand/MoveElementCommand/DeleteElementCommand)"
E->>S : "执行命令对应操作"
E->>H : "push(command)"
E->>E : "notify('scene')"
E->>E : "notify('history')"
H-->>E : "记录成功"
E-->>C : "完成"
C-->>U : "UI 刷新"
```

**图表来源**
- [src/components/Canvas.tsx:44-77](file://src/components/Canvas.tsx#L44-L77)
- [src/engine/engine.ts:88-105](file://src/engine/engine.ts#L88-L105)
- [src/engine/commands.ts:4-68](file://src/engine/commands.ts#L4-L68)
- [src/engine/scene.ts:94-159](file://src/engine/scene.ts#L94-L159)

**章节来源**
- [src/engine/engine.ts:88-105](file://src/engine/engine.ts#L88-L105)
- [src/engine/history.ts:7-30](file://src/engine/history.ts#L7-L30)
- [src/engine/commands.ts:20-68](file://src/engine/commands.ts#L20-L68)
- [src/components/Canvas.tsx:44-77](file://src/components/Canvas.tsx#L44-L77)

### 动画配置与分步播放（AnimationEngine + Scheduler）
- App 在右侧面板切换到"动画"时，根据当前页动画列表构建 AnimationEngine 配置并创建 AnimationScheduler。
- Scheduler 将动画按"点击步骤（Step）+ 并发批次（Batch）"组织，Step 内部批次顺序执行，批次内动画并发播放。
- 用户点击"下一步/上一步"推进或回退步骤，Scheduler 负责取消未完成控制器并重放当前步骤。

```mermaid
sequenceDiagram
participant U as "用户"
participant APP as "App.tsx"
participant AE as "AnimationEngine"
participant SCH as "AnimationScheduler"
participant EL as "元素DOM"
U->>APP : "切换到动画面板"
APP->>AE : "reset() + register(启用动画)"
APP->>SCH : "new AnimationScheduler(AE)"
SCH->>SCH : "load(Enabled Anims)"
U->>APP : "点击 下一步"
APP->>SCH : "playNextStep()"
SCH->>AE : "play(批次内动画)"
AE->>EL : "适配器播放"
EL-->>AE : "完成回调"
AE-->>SCH : "通知完成"
SCH-->>APP : "更新进度"
```

**图表来源**
- [src/App.tsx:40-105](file://src/App.tsx#L40-L105)
- [src/animation/engine.ts:32-118](file://src/animation/engine.ts#L32-L118)
- [src/animation/scheduler.ts:56-160](file://src/animation/scheduler.ts#L56-L160)

**章节来源**
- [src/App.tsx:40-105](file://src/App.tsx#L40-L105)
- [src/animation/engine.ts:32-118](file://src/animation/engine.ts#L32-L118)
- [src/animation/scheduler.ts:56-160](file://src/animation/scheduler.ts#L56-L160)

### 数据一致性与状态同步
- Scene 的元素更新会自动维护父子关系与组内子项列表，避免悬挂引用。
- EditorState 与 Scene 分离，通过版本号驱动 React 刷新，避免跨层状态污染。
- App 使用 useEffect 同步 Scene 动画到 AnimationEngine，确保编辑态与播放态一致。
- **新增**：精确订阅机制确保组件只在相关数据变化时重渲染，避免全局重渲染。

```mermaid
flowchart TD
Start(["开始"]) --> ReadDoc["读取当前文档"]
ReadDoc --> GetCurrentPage["获取当前页ID"]
GetCurrentPage --> FetchAnims["获取当前页动画数组"]
FetchAnims --> ResetAE["AnimationEngine.reset()"]
ResetAE --> Register["逐个register(启用动画)"]
Register --> Done(["结束"])
```

**图表来源**
- [src/App.tsx:32-37](file://src/App.tsx#L32-L37)
- [src/animation/engine.ts:115-118](file://src/animation/engine.ts#L115-L118)
- [src/engine/scene.ts:212-233](file://src/engine/scene.ts#L212-L233)

**章节来源**
- [src/engine/scene.ts:108-135](file://src/engine/scene.ts#L108-L135)
- [src/App.tsx:32-37](file://src/App.tsx#L32-L37)

### 复杂逻辑：动画分步编排（Scheduler）
- buildClickSteps 将动画序列转换为 ClickStep 列表，依据 startType 决定是否开启新 Step 或新 Batch。
- executeBatch 并发启动批次内动画，监听完成回调以推进下一个批次；若无未完成任务则直接进入下一阶段。

```mermaid
flowchart TD
A["接收动画数组"] --> B{"startType"}
B --> |click| NewStep["新建 Step + 新 Batch"]
B --> |withPrev| JoinBatch["加入当前 Batch若存在"]
B --> |afterPrev| NewBatch["新建 Batch若存在 Step"]
NewStep --> C["累积步骤"]
JoinBatch --> C
NewBatch --> C
C --> D["返回 ClickStep[]"]
```

**图表来源**
- [src/animation/scheduler.ts:13-49](file://src/animation/scheduler.ts#L13-L49)

**章节来源**
- [src/animation/scheduler.ts:56-160](file://src/animation/scheduler.ts#L56-L160)

### 类型与数据模型
- 元素类型（形状/文本/图片/组）与页面结构（pages、nodes、structureItems、currentPageId）由 Document 统一承载。
- 动画配置（AnimationConfig）与调度模型（ClickStep、AnimationBatch）独立于引擎的 Timeline 动画类型。
- **新增**：EngineTopic类型定义了精确订阅的主题系统。

```mermaid
erDiagram
DOCUMENT {
string id
string name
string current_page_id
}
PAGE {
string id
string name
string background
}
NODE {
string id
string name
}
ELEMENT {
string id
string type
string parent_id
}
ANIMATION_CONFIG {
string id
string element_id
string name
boolean enable
string start_type
}
DOCUMENT ||--o{ PAGE : "包含"
DOCUMENT ||--o{ NODE : "包含"
PAGE ||--o{ ELEMENT : "包含"
PAGE ||--o{ ANIMATION_CONFIG : "包含"
```

**图表来源**
- [src/types/index.ts:77-84](file://src/types/index.ts#L77-L84)
- [src/types/index.ts:69-75](file://src/types/index.ts#L69-L75)
- [src/types/index.ts:64-67](file://src/types/index.ts#L64-L67)
- [src/types/animation.ts:26-39](file://src/types/animation.ts#L26-L39)

**章节来源**
- [src/types/index.ts:10-159](file://src/types/index.ts#L10-L159)
- [src/types/animation.ts:26-113](file://src/types/animation.ts#L26-L113)
- [src/engine/engine.ts:7](file://src/engine/engine.ts#L7)

## 依赖关系分析
- App.tsx 依赖 Engine 与 AnimationEngine，负责状态刷新与动画调度。
- Canvas.tsx 仅通过 Engine 与命令进行交互，不直接访问 AnimationEngine，保持职责单一。
- Engine 依赖 Scene、History、Timeline 与 Commands，形成稳定的内聚模块。
- AnimationEngine 依赖适配器接口，通过类型系统与 buildKeyframes 解耦具体播放库。
- **新增**：Store体系依赖Engine的精确订阅机制，提供主题化的状态管理。

```mermaid
graph LR
APP["App.tsx"] --> ENG["Engine"]
APP --> ANIM["AnimationEngine"]
CANV["Canvas.tsx"] --> ENG
ENG --> SCN["Scene"]
ENG --> HIS["History"]
ENG --> TLI["Timeline"]
ENG --> CMD["Commands"]
BASE["BaseStore"] --> ENG
ENG --> BASE
SCNSTORE["SceneStore"] --> BASE
SELSTORE["SelectionStore"] --> BASE
UISTORE["EditorUIStore"] --> BASE
HISSTORE["HistoryStore"] --> BASE
ANIMSTORE["AnimationStore"] --> BASE
ANIM --> ADP["AnimationAdapter"]
```

**图表来源**
- [src/App.tsx:19-335](file://src/App.tsx#L19-L335)
- [src/components/Canvas.tsx:22-191](file://src/components/Canvas.tsx#L22-L191)
- [src/engine/engine.ts:15-128](file://src/engine/engine.ts#L15-L128)
- [src/store/baseStore.ts:1-51](file://src/store/baseStore.ts#L1-L51)
- [src/animation/engine.ts:9-120](file://src/animation/engine.ts#L9-L120)

**章节来源**
- [src/App.tsx:19-335](file://src/App.tsx#L19-L335)
- [src/components/Canvas.tsx:22-191](file://src/components/Canvas.tsx#L22-L191)
- [src/engine/engine.ts:15-128](file://src/engine/engine.ts#L15-L128)
- [src/store/baseStore.ts:1-51](file://src/store/baseStore.ts#L1-L51)
- [src/animation/engine.ts:9-120](file://src/animation/engine.ts#L9-L120)

## 性能考量
- 命令执行与历史栈：命令对象轻量，仅保存必要上下文，避免深拷贝开销。
- 动画播放：AnimationEngine 仅在需要时查询 DOM，且通过 setScopeRoot 限定查询范围，降低选择器成本。
- 分步播放：Scheduler 的并发与串行控制减少不必要的等待，提升交互流畅度。
- UI 刷新：通过精确订阅机制，组件只在关心的数据变化时重渲染，避免全量 diff。
- **新增**：三层订阅隔离机制显著减少了不必要的组件重渲染，提升了整体性能。

## 故障排查指南
- 撤销/重做无效
  - 检查 History 栈是否为空，确认 Engine.canUndo()/canRedo() 返回值。
  - 确认命令实现了正确的 undo() 行为。
- 删除元素无效
  - 确认 EditorState.selectedElementIds 是否正确设置，键盘事件处理是否拦截。
  - 检查 Scene.deleteElement 是否被调用且元素存在于当前页。
- 动画不播放
  - 确认 AnimationEngine.register 的动画处于启用状态，元素 ID 与 DOM 属性匹配。
  - 检查 setScopeRoot 是否指向正确的容器，适配器是否可用。
- 步骤播放异常
  - 检查 buildClickSteps 的 startType 是否符合预期，Scheduler 的步骤索引与控制器清理是否正确。
- **新增**：精确订阅问题排查
  - 检查Store实例化时的主题列表是否正确配置。
  - 确认组件使用的Store是否订阅了正确的主题。
  - 验证Engine.notify是否按预期主题分发。

**章节来源**
- [src/engine/history.ts:12-30](file://src/engine/history.ts#L12-L30)
- [src/engine/commands.ts:46-68](file://src/engine/commands.ts#L46-L68)
- [src/App.tsx:108-150](file://src/App.tsx#L108-L150)
- [src/engine/scene.ts:137-159](file://src/engine/scene.ts#L137-L159)
- [src/animation/engine.ts:24-30](file://src/animation/engine.ts#L24-L30)
- [src/animation/scheduler.ts:115-133](file://src/animation/scheduler.ts#L115-L133)
- [src/store/readme.md:90-142](file://src/store/readme.md#L90-L142)

## 结论
该数据流系统以命令为中心，结合 Scene 文档状态与 History 历史栈，确保所有变更可追踪、可回滚；通过 AnimationEngine 与 Scheduler 实现动画的分步与并发控制，配合类型系统与适配器解耦，达成高内聚、低耦合的架构目标。**新增的精确订阅理论**通过Engine-level、Store-level和React-level三个层级的订阅隔离机制，显著提升了系统的性能和可维护性，确保组件只在关心的数据变化时才重渲染。通过版本号驱动的 UI 刷新与作用域限定的 DOM 查询，兼顾了可维护性与性能表现。

## 附录
- 关键流程参考路径
  - 命令执行与撤销：[src/engine/engine.ts#L88-L105:88-105](file://src/engine/engine.ts#L88-L105)，[src/engine/history.ts#L12-L30:12-30](file://src/engine/history.ts#L12-L30)
  - 元素编辑命令：[src/engine/commands.ts#L4-L68:4-68](file://src/engine/commands.ts#L4-L68)
  - 动画注册与播放：[src/animation/engine.ts#L32-L118:32-118](file://src/animation/engine.ts#L32-L118)
  - 分步播放编排：[src/animation/scheduler.ts#L56-L160:56-160](file://src/animation/scheduler.ts#L56-L160)
  - 类型定义：[src/types/index.ts#L107-159:107-159](file://src/types/index.ts#L107-L159)，[src/types/animation.ts#L26-113:26-113](file://src/types/animation.ts#L26-L113)
  - **新增**：精确订阅机制：[src/engine/engine.ts#L7-L72:7-72](file://src/engine/engine.ts#L7-L72)，[src/store/baseStore.ts#L39-L49:39-49](file://src/store/baseStore.ts#L39-L49)，[src/store/readme.md#L5-L46:5-46](file://src/store/readme.md#L5-L46)