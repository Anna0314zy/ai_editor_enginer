...
- 新增页面背景渲染功能，支持纯色、渐变和图片背景
- 新增视觉安全区域指南功能，帮助设计师在内容边界内工作
- 新增getBackgroundStyle()辅助函数处理背景样式生成
...
**更新** Canvas组件现已采用新的Store架构，通过SceneStore和SelectionStore提供响应式的状态管理，替代原有的全局Engine快照访问模式。新增页面背景渲染和视觉安全区域指南功能，提升设计体验。
...
**更新** 新增Store架构，通过SceneStore和SelectionStore提供响应式状态管理，替代原有的全局Engine快照访问。新增页面背景渲染和视觉安全区域指南功能。

**章节来源**
- [Canvas.tsx:15-185](file://src/components/Canvas.tsx#L15-L185)
...
- 核心职责
  ...
  - **新增** 视觉安全区域指南：根据文档安全区域配置渲染不安全区域覆盖层和安全边界线。

**更新** 现在通过useStores钩子访问SceneStore和SelectionStore，替代原有的直接引擎访问。新增页面背景渲染和视觉安全区域指南功能。
...
### 页面背景渲染功能
- getBackgroundStyle() 辅助函数
  - 处理页面背景配置并生成CSS样式对象。
  - 支持三种背景类型：纯色、渐变和图片。
  - 默认返回白色背景，确保兼容性。
- 背景类型支持
  - 纯色背景：backgroundColor属性设置颜色值。
  - 渐变背景：backgroundImage属性设置线性渐变，angle角度和stops颜色停止点。
  - 图片背景：backgroundImage设置图片URL，fit控制填充方式，opacity控制透明度。
- 背景应用
  - 通过getBackgroundStyle()函数获取样式对象，应用到画布背景层。
  - 支持页面级背景覆盖文档级背景。

**新增** 新增页面背景渲染功能，支持多种背景类型配置。

**章节来源**
- [Canvas.tsx:12-32](file://src/components/Canvas.tsx#L12-L32)
- [Canvas.tsx:140-146](file://src/components/Canvas.tsx#L140-L146)
- [index.ts:72-97](file://src/types/index.ts#L72-L97)

### 视觉安全区域指南功能
- 安全区域配置
  - 支持top、right、bottom、left四个方向的安全边距。
  - 提供标准预设：无、20px、40px、Action Safe 5%。
- 不安全区域覆盖层
  - 四个方向的红色半透明覆盖层，提示内容超出安全区域的部分。
  - 使用rgba(239, 68, 68, 0.08)颜色，轻微遮挡不影响内容阅读。
- 安全边界线
  - 蓝色虚线边框，透明度0.5，突出显示安全区域边界。
  - 边界线位于安全区域内部，帮助设计师准确定位内容范围。
- 条件渲染
  - 当安全区域配置为全零时，不显示任何指南线。
  - 仅在存在有效安全区域配置时才渲染指南功能。

**新增** 新增视觉安全区域指南功能，帮助设计师在内容边界内工作。

**章节来源**
- [Canvas.tsx:147-173](file://src/components/Canvas.tsx#L147-L173)
- [index.ts:92-97](file://src/types/index.ts#L92-L97)
- [GlobalSettingsPanel.tsx:11-16](file://src/components/GlobalSettingsPanel.tsx#L11-L16)
...
- [commands.ts:224-254](file://src/engine/commands.ts#L224-L254)
...
- **新增** 页面背景类型
  - PageBackgroundSolid：纯色背景，包含颜色值。
  - PageBackgroundGradient：渐变背景，包含角度和颜色停止点数组。
  - PageBackgroundImage：图片背景，包含URL、填充方式和透明度。
- **新增** 安全区域类型
  - SafeArea：包含四个方向的边距值，用于视觉安全区域指南。

**新增** 新增页面背景和安全区域的数据类型定义。
...
- **新增** 背景渲染优化
  - getBackgroundStyle()函数使用条件判断和样式合并，避免不必要的DOM操作。
  - 安全区域指南使用条件渲染，只有在配置有效时才显示。

**更新** 新的Store架构提供了更好的性能表现，通过细粒度的订阅机制和快照缓存减少了不必要的重渲染。新增背景渲染和安全区域指南的性能优化。
...
- Store访问错误
  ...
  - 检查useStores钩子是否正确调用。
- **新增** 背景渲染问题
  - 检查PageBackground配置格式是否正确，包括type、color、angle、stops、src、fit、opacity等字段。
  - 确认getBackgroundStyle()函数能够正确解析背景配置。
- **新增** 安全区域指南问题
  - 检查SafeArea配置是否为有效的数值，top、right、bottom、left都应为非负数。
  - 确认安全区域边界线不会与内容重叠，透明度设置合理。

**更新** 新增Store相关的故障排除指南，包括上下文访问和状态管理问题。新增背景渲染和安全区域指南的故障排除指南。
...
## 结论
Canvas 组件通过新的Store架构实现了拖拽放置、元素选择、渲染与交互控制。新的SceneStore和SelectionStore提供了响应式的状态管理，替代原有的全局Engine快照访问模式，提升了代码的可维护性和性能。借助命令模式与历史管理，系统具备良好的可撤销/重做能力；通过吸附与引导线提升了编辑体验。**新增的页面背景渲染功能支持多种背景类型配置，视觉安全区域指南帮助设计师在内容边界内工作，进一步提升了设计体验和专业性。**整体设计遵循关注点分离原则，便于扩展与维护。

**更新** 新的Store架构显著改进了状态管理的清晰度和性能表现，为后续功能扩展奠定了坚实基础。新增的背景渲染和安全区域指南功能进一步完善了画布的专业设计能力。
...
- 画布背景设置
  ...
  - 支持纯色、渐变和图片背景，通过getBackgroundStyle()函数统一处理。
- 响应式设计
...
- **新增** 背景配置最佳实践
  - 使用GlobalSettingsPanel或PropertyPanel配置页面背景。
  - 渐变背景建议使用135度角度获得自然过渡效果。
  - 图片背景注意设置合适的fit模式和透明度。
- **新增** 安全区域配置最佳实践
  - Action Safe 5%适用于大多数视频内容，确保字幕和重要元素不被裁剪。
  - 标准20px适用于一般设计内容，提供舒适的边距空间。
  - 自定义安全区域时，确保四个方向的数值合理分配。

**更新** 新增Store架构的使用指南和最佳实践。新增背景渲染和安全区域指南的使用指南和最佳实践。
...
- [StoreProvider.tsx:26-40](file://src/store/StoreProvider.tsx#L26-L40)
- [hooks.ts:9-15](file://src/store/hooks.ts#L9-L15)
- [GlobalSettingsPanel.tsx:78-136](file://src/components/GlobalSettingsPanel.tsx#L78-L136)